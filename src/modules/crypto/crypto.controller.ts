...
export class CryptoController {
  ...
  async getLatestPortfolio(req: Request, res: Response) {
    try {
      ...
      robustLogger.error("Error in getLatestPortfolio:", error);
  ...
  async getNavHistory(req: Request, res: Response) {
    try {
      ...
      robustLogger.info("Error in getNavHistory:", error);
      return res.status(HTTPStatusCode.InternalServerError).json({
        success: false,
  ...
  async getAssetPerformance(req: Request, res: Response) {
    try {
      ...
      robustLogger.info("Error in getAssetPerformance:", error);
      return res.status(HTTPStatusCode.InternalServerError).json({
        success: false,
  ...
  async getChartData(req: Request, res: Response) {
    try {
      ...
      robustLogger.info("Error in getChartData:", error);
      return res.status(HTTPStatusCode.InternalServerError).json({
        success: false,
  ...
  async getSystemStatus(req: Request, res: Response) {
    try {
      ...
      robustLogger.info("Error in getSystemStatus:", error);
      return res.status(HTTPStatusCode.InternalServerError).json({
        success: false,
  ...
  async getCurrentPrices(req: Request, res: Response) {
    try {
      ...
      robustLogger.info("Error in getCurrentPrices:", error);
      return res.status(HTTPStatusCode.InternalServerError).json({
        success: false,
  ...
  async getPortfolioSummary(req: Request, res: Response) {
    try {
      ...
      const summary = {
        ...
        allocation_breakdown: Object.entries(latestData.allocations).map(
          ([key, allocation]) => ({
            key,
      ...
      robustLogger.info("Error in getPortfolioSummary:", error);
      return res.status(HTTPStatusCode.InternalServerError).json({
        success: false,
  ...
  async triggerManualUpdate(req: Request, res: Response) {
    try {
      ...
      robustLogger.info("Error in triggerManualUpdate:", error);
      return res.status(HTTPStatusCode.InternalServerError).json({
        success: false,
  ...
  async getHealthCheck(req: Request, res: Response) {
    try {
      const latestData = await this.service.getLatestData();
      const isHealthy =
        latestData && latestData.system_status.last_sync_success;
      ...
      robustLogger.info("Error in getHealthCheck:", error);
      return res.status(HTTPStatusCode.InternalServerError).json({
        success: false,
